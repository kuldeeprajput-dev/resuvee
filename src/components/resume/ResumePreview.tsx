  ...
  return (
    <Sheet className={cn("bg-white px-8 py-7 font-serif text-[#202020]", className)}>
      <header className="text-center">
        ...
        <p className="mt-1.5 text-[14px] font-semibold leading-none">
      ...
      <main className="mt-5 space-y-4">
        ...
        {data.skillGroups.length > 0 && (
          <section>
            ...
            <div className="space-y-1 text-[8.7px] leading-[1.35] text-black/75">
              {data.skillGroups.map((group) => (
                <p key={group.id}>
              ...
              {data.experience.map((item) => (
                <article key={item.id}>
                  <div className="flex items-baseline justify-between gap-4">
                    <h3 className="text-[9.5px] font-bold">{item.role}</h3>
                    <p className="shrink-0 text-[8.5px] font-bold">
                      {[item.startDate, item.current ? "Present" : item.endDate]
                        .filter(Boolean)
                        .join(" – ")}
                    </p>
                  </div>
                  <div className="mt-0.5 flex items-baseline justify-between gap-4 text-[8px] italic text-black/65">
                    <p>{item.company}</p>
                    <p className="text-right">{item.location}</p>
                  </div>
                  <ul className="mt-1.5 space-y-0.5 pl-4 text-[8.2px] leading-[1.34] text-black/72">
                    {item.highlights.filter(Boolean).map((highlight, index) => (
          ...
          <section>
            ...
            <div className="space-y-3">
              {data.projects.map((project) => (
                <article key={project.id}>
                  <div className="flex items-baseline justify-between gap-4">
                    <h3 className="min-w-0 text-[9.2px] font-bold">
                      {project.name}
                      {project.description && (
        ...
        {certifications.length > 0 && (
          <section>
            ...
            <ul className="space-y-1 pl-4 text-[8.1px] leading-[1.35] text-black/72">
      ...
      <p className="absolute bottom-4 right-8 text-[5px] uppercase tracking-[0.14em] text-black/20">
        Resulyra · {template.name}
      </p>
    </Sheet>
  );
