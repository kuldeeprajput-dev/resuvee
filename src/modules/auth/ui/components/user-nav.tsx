  ...
  if (!user) {
    return (
      <button
        ...
        className="inline-flex h-9 items-center justify-center gap-1.5 rounded-full px-3.5 text-[13px] font-semibold text-(--brand-ink) transition-colors hover:bg-black/5 cursor-pointer"
      >
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      >
        {avatarUrl && !imageError ? (
          ...
          <div className="flex size-5 items-center justify-center rounded-full bg-(--brand-ink) text-[10px] font-extrabold text-white shrink-0">
            {userInitial}
          </div>
        )}
        ...
        <div className="absolute right-0 top-11 z-50 w-56 rounded-2xl border border-black/10 bg-white/95 p-2 shadow-[0_10px_35px_rgba(0,0,0,0.09)] backdrop-blur-xl animate-in fade-in zoom-in-95">
          ...
          <div className="p-1 space-y-0.5">
            <Link
              ...
              className="flex w-full items-center gap-2 rounded-xl px-2.5 py-2 text-xs font-semibold text-(--brand-ink) hover:bg-black/5 transition"
            >
              <FileText className="size-3.5 text-(--brand-muted)" />
