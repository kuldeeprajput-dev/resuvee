...
export function TailorPanel({
  ...
  const result = useMemo(
    () => analyzeJobMatch(data, description),
    [data, description],
  );
  const suggested = result.missing
    .filter((keyword) => !added.includes(keyword))
    .slice(0, 8);
      ...
      <aside className="relative flex h-full w-full max-w-[520px] flex-col overflow-hidden bg-[#f7f6f1] shadow-2xl">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5 sm:p-7">
          ...
          <p className="mt-1 text-xs leading-5 text-[var(--brand-muted)]">
            Resulyra checks role language locally in your browser. Nothing is
            uploaded.
          </p>
          <textarea
            id="job-description"
